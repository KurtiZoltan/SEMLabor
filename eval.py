...
plt.imshow(np.log(np.abs(transform)**2), extent=((-2560/2-0.5) * k0x, (2560/2-0.5) * k0x, (-1920/2-0.5) * k0y, (1920/2-0.5) * k0y), origin="lower", vmax=32.5)
plt.xlabel("$k_x$ [$m^{-1}$]")
plt.ylabel("$k_y$ [$m^{-1}$]")
plt.show()
plt.imshow(np.log(np.abs(transform[xmin:xmax, ymin:ymax])**2), origin="lower", aspect=k0y/k0x, extent=(-r-0.5, r-0.5, -r-0.5, r-0.5), vmax=32.5)
